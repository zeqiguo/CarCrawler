...
class Crawler:
    ...
    def get_makes(self) -> list:
        ...
        return: Acura
        '''
        # return the makes as a list, prepare for further crawl
        # the return values in the list was removed all '/'
...
#     def split_file(self):
#         # split sub_problem.npy to files which include 10k each
#         sub_p = np.load('sub_problems.npy')

#         i = 0
#         j = 1
#         while i < len(sub_p):
#             i += 1000
#             np.save(file='sub_p/sub%d.npy' % j, arr=sub_p[:i])
#             j += 1
    ...
    def get_all_comments(self):
        ...
        with open('CarV3.csv', mode='w', encoding='utf-8') as carFile:
...
# np.save(file='sub_1.npy', arr=a[6810:7890])
np.save(file='sub_2.npy', arr=a[7890:])
